...
const CreateAssetForm = () => {
  ...
  const onFinish = (values) => {
    console.log(values);
    api.post('assets', {token: window.localStorage.getItem('company_id'), 
    unity: values.unity_ID, image: values.image, name: values.name, description: values.description, 
    model: values.model, state: values.state, healthscore: values.healthscore}).then(() => {
      setTimeout(() => {
  ...
  return (
    <div className="container">
      <h1>(Novo Ativo) Preencha os campos abaixo:</h1>
      ...
      >
        <Form.Item name="unity_ID" label="Unidade" rules={[
            {
              required: true,
              message:
