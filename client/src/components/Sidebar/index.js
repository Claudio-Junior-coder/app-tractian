...
export default function Sidebar() {
  ...
  function renderCompanyItems() {
    return listNamesCompany.map((val) => {
      return (
    ...
    <Sider collapsible collapsed={collapsed} onCollapse={onCollapse}>
      ...
      >
        <Form.Item className="select-main">
          ...
          {persistedCompanyId ? (
            <Select defaultValue={persistedCompanyId} onChange={saveCompanyId}>
              {renderCompanyItems()}
            </Select>
          ) : (
            <Select onChange={saveCompanyId}>{renderCompanyItems()}</Select>
          )}
        </Form.Item>
      </Form>
      ...
      <Menu theme="dark" defaultSelectedKeys={["1"]} mode="inline">
        ...
        <SubMenu key="sub1" icon={<BookOutlined />} title="Empresas">
          ...
          <Menu.Item key="2" onClick={handleUpdateCompany}>
            Editar
          </Menu.Item>
          <Menu.Item key="3" onClick={handleDeleteCompany}>
            Excluir
          </Menu.Item>
        </SubMenu>
        <SubMenu key="sub2" icon={<HomeOutlined />} title="Unidades">
          <Menu.Item key="4" onClick={handleCreateUnity}>
            Adicionar
          </Menu.Item>
          <Menu.Item key="5" onClick={handleUpdateUnity}>
            Editar
          </Menu.Item>
          <Menu.Item key="6" onClick={handleDeleteUnity}>
            Excluir
          </Menu.Item>
        </SubMenu>
        <SubMenu key="sub3" icon={<ProfileOutlined />} title="Ativos">
          <Menu.Item key="7" onClick={handleCreateAsset}>
            Adicionar
          </Menu.Item>
          <Menu.Item key="8" onClick={handleUpdateAsset}>
            Editar
          </Menu.Item>
          <Menu.Item key="9" onClick={handleDeleteAsset}>
            Excluir
          </Menu.Item>
        </SubMenu>
        <SubMenu key="sub4" icon={<UserAddOutlined />} title="Responsáveis">
          <Menu.Item key="10" onClick={handleCreateResponsible}>
            Adicionar
          </Menu.Item>
          <Menu.Item key="11" onClick={handleUpdateResponsible}>
            Editar
          </Menu.Item>
          <Menu.Item key="12" onClick={handleDeleteResponsable}>
            Excluir
          </Menu.Item>
        </SubMenu>
      </Menu>
